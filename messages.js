...
function showDidYouMean(suggestions, options) {
  // run this if confidence is < 0.9
...
function buildFullQueryLabel(intentName, entityValue) {
  ...
  const intent =
    typeof INTENTS !== "undefined" && Array.isArray(INTENTS)
      ? INTENTS.find(function (i) {
          return i && i.name === intentName;
        })
      : null;
  let template = null;
  if (intent && Array.isArray(intent.phrases) && intent.phrases.length > 0) {
    const showPhrase = intent.phrases.find(function (p) {
      return /^show\b/i.test(String(p || ""));
    });
    template = showPhrase || intent.phrases[0];
  }
  if (!template) {
    if (intentName === "list_po_vendor") template = "List all POs from X";
    else if (intentName && intentName.indexOf("list") === 0)
      template = "List all POs from X";
    else template = String(entityValue || "");
  }

  let label = String(template || "")
    .replace(/\bX\b/gi, String(entityValue || ""))
    .replace(/\s+/g, " ")
    .trim();
...
function getAccessDeniedMessage_() {
  return "You do not have access to this chatbot. Please contact the admin team at ntg-bmsocapexsettlement@globe.com.ph for further assistance.";
}
...
function getCommschedNotFoundMessage_(poNumber) {
  return (
    "Cannot find <b>PO " +
    poNumber +
    "</b>. Please contact the admin team at ntg-bmsocapexsettlement@globe.com.ph for further assistance."
  );
}
...
function buildPoAgingReply_(poNumber, bucketInfo, intentName) {
  ...
  if (intentName === "check_po_aging_exceeded") {
    if (bucketCode === "d" || bucketCode === "e") {
      return (
        ...
        " is " +
        bucketLabel +
        " old from the PO creation date. It has exceeded the standard SLA."
      );
    }
    ...
    return (
      ...
      " is " +
      bucketLabel +
      " old from the PO creation date. It has not yet exceeded the standard SLA."
    );
  }
  ...
  if (intentName === "check_po_high_risk") {
    if (bucketCode === "d") {
      return (
        ...
        " is " +
        bucketLabel +
        " old from the PO creation date. It is a high risk PO."
      );
    }
    ...
    if (bucketCode === "e") {
      return (
        boldPo +
        " is >24 months old from the PO creation date. It is a high risk legacy PO."
      );
    }
    ...
    return (
      ...
      " is " +
      bucketLabel +
      " old from the PO creation date. It is not yet a high risk PO."
    );
  }
  ...
  if (bucketCode === "e") {
    return (
      boldPo +
      " is >24 months old from the PO creation date. It is already a high risk legacy PO."
    );
  }
...
function finalizeBotResponse_(response, userProfile) {
  if (
    ...
    const errorColumn =
      typeof emailColumns.errors === "number" ? emailColumns.errors + 1 : 6;
...
function buildTableResponse_(headers, rows, options) {
  ...
  const headerCells = Array.isArray(headers) ? headers : [];
  const maxDisplayRows = Number.isInteger(config.maxDisplayRows)
  ...
  if (hasOverflow && maxDisplayRows > 0 && config.showOverflowRow !== false) {
    ...
    visibleRows = allRows
      .slice(0, visibleDataRows)
      .concat([
        buildOverflowTableRow_(
          headerCells.length,
          "+" + (allRows.length - visibleDataRows) + " more rows",
        ),
      ]);
  }
